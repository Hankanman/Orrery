//! A single-line text input — adapted from gpui's `examples/input.rs`, themed for
//! Orrery and reduced to what the app needs. Keeps the proven `EntityInputHandler`
//! (correct IME / unicode input) and the custom element that shapes the line and
//! positions the cursor; supports selection, clipboard, and mouse caret placement.
//!
//! Use it as its own entity: `cx.new(|cx| TextInput::new(cx, style, "placeholder"))`,
//! render it with `.child(entity.clone())`, and read `entity.read(cx).content()`.
//! Key bindings for its actions are registered once in `main`.
...
/// Register the input key bindings (call once from `main`).
...
/// Colours + size for an input, captured from the theme at construction (the
/// input is a `'static` entity and can't hold a `&Theme`).
#[derive(Clone, Copy)]
...
impl TextInput {
    ...
    fn on_mouse_down(
        ...
        // Clicking the field focuses it so typing works.
...
impl gpui::Render for TextInput {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .bg(rgb(s.bg))
            .border_1()
            .border_color(rgb(s.border))
            .text_size(px(s.size))
            .text_color(rgb(s.text))
            .line_height(px(line_h))
            .child(InputElement { input: cx.entity() })
    }
}

/// The custom element that shapes the single line and paints the cursor/selection.
struct InputElement {
...
impl Element for InputElement {
    ...
    fn prepaint(
        ...
        let (display_text, color) = if content.is_empty() {
            (input.placeholder.clone(), style.placeholder)
        } else {
            (content, style.text)
        };
